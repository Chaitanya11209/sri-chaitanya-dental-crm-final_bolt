...
export default function Treatments() {
  ...
  useEffect(() => {
    ...
    const channelA = supabase
      .channel('treatments-appointments-realtime')
  ...
  const fetch = async () => {
    setLoading(true);
    try {
      const { data: treatmentsData } = await supabase.from('treatments').select('*').order('created_at', { ascending: false });
      const { data: appointmentsData } = await supabase.from('appointments').select('*').neq('status', 'Deleted').order('created_at', { ascending: false });
