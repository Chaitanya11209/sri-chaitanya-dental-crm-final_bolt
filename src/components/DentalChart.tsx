...
export type ToothStatus = 'Healthy' | 'Caries' | 'Filled' | 'RCT Done' | 'Crown' | 'Missing' | 'Extraction Planned' | 'Implant Planned' | 'Implant Completed';
...
const TOOTH_STATUSES: { id: ToothStatus; label: string; color: string; border: string; desc: string }[] = [
  ...
  { id: 'RCT Done', label: 'RCT Done', color: 'bg-violet-500', border: 'border-violet-600', desc: 'Root Canal Treatment done' },
  ...
  { id: 'Extraction Planned', label: 'Extraction Planned', color: 'bg-orange-500', border: 'border-orange-600', desc: 'Removal scheduled' },
  { id: 'Implant Planned', label: 'Implant Planned', color: 'bg-indigo-500', border: 'border-indigo-600', desc: 'Implant scheduled' },
  { id: 'Implant Completed', label: 'Implant Completed', color: 'bg-teal-600', border: 'border-teal-700', desc: 'Successful implant prosthetic' },
];
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${chartType === 'adult' ? 'bg-white shadow-xs text-teal-700' : 'text-slate-505 hover:text-slate-705'}`}
          >
            Adult Chart (1-32)
          </button>
          <button
            onClick={() => { setChartType('child'); setSelectedTooth(null); }}
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${chartType === 'child' ? 'bg-white shadow-xs text-teal-700' : 'text-slate-505 hover:text-slate-705'}`}
          >
            Child Chart (A-T)
          </button>
        </div>
