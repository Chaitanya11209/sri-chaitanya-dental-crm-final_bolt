...
import { CLINIC_SIGNATURE } from '../utils/whatsapp';
...
export const DEFAULT_SMS_TEMPLATES: SMSTemplates = {
  appointment: `Hi [Name], your appointment for [Treatment] is scheduled on [Date] at [Time]. Thank you!\n\n${CLINIC_SIGNATURE}`,
...
export async function sendSMS({ phone, name, message }: SendSMSParams): Promise<SMSResponse> {
  ...
  // Automatically append signature if not already present block
  const fullMessage = message.includes("8317575165")
    ? message
    : `${message.trim()}\n\n${CLINIC_SIGNATURE}`;

  const activeChannel = getSMSChannel();

  console.log(`[SMS SYSTEM] Dispatching Message via [${activeChannel.toUpperCase()}] to ${name} (${formattedPhone}). Content: "${fullMessage}"`);

  // --- FREE ROUTING OPTIONS ---

  if (activeChannel === 'whatsapp') {
    const waUrl = `https://wa.me/${formattedPhone}?text=${encodeURIComponent(fullMessage)}`;
  ...
  if (activeChannel === 'device') {
    const smsUrl = `sms:${formattedPhone}?body=${encodeURIComponent(fullMessage)}`;
